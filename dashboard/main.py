...
def update_charts(selected_period):
    ...
    # Times que mais venceram campeonatos durante o período
    most_championships_fig = px.bar(filtered_df.groupby('team')['place'].min().sort_values(ascending=True).head(5).reset_index(),
    ...
    # Times que mais fizeram gols contra durante o período
    most_goals_against_fig = px.bar(filtered_df.groupby('team')['goals_against'].sum().sort_values(ascending=False).head(5).reset_index(),
